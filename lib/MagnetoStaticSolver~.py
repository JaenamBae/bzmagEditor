...
class MagnetoStaticSolver:
    ...
    def loadBHCurve(self, file):
        ...
        for line in rdr:
            ...
            # 원점대칭형 곡선으로 만들어 둔다(추후 미분문제 해결위해)
            if B != 0: bh_data.insert(0, [-B, -H])
        f.close()
        ...
        inter_bhcurve = interp1d(data_x, data_y)
        
        # B2-v 데이터 생성 --> 선형보간을 해야 맞나? 스플라인보간을 해야 맞나?
        # 선형보간이 나을것 같다. 해의 수렴문제와 직결됨!
        b2v_data = list()
        for B, H in bh_data:
            if B == 0 : v = 0
            else: v = H/B
            ...
            #if len(b2v_data)>0:
            #    B2v_pre = b2v_data[-1]
            #    if (v <= B2v_pre[1]) or (B2 <= B2v_pre[0]) :
            #        print('BH curve (',file, ') has no Integrity!! Solution could not be converged')
            if B >= 0: b2v_data.append([B2, v])
            else : b2v_data.append([-B2, -v])
        ...
        # v data
        data_y = B2vCurve[:,1]
        # spline보간을 하게 되면 그래프가 단조증가 함수가 안될 확률이 높다
        # B2v그래프는 NR법 적용시 반드시 단조증가 함수이어야 한다
        # (그래프의 모든곳에서의 기울기가 0보다 무조건 커야함)
    ...
    #-------------------------------------------------------------------------
    def fp(self, inter_curve, x):
        eps = 1e-5
        return (inter_curve(x+eps) - inter_curve(x))/eps
        ...
        for e in self.FE_Elements_.values():
            ...
            for i, w in enumerate(wi): 
                ...
                if BH != None:
                    ...
                    absBx = np.abs(Bx)
                    #print(absBx)
                    if absBx == 0 : nu_x = BH(0.001) / 0.001
                    else : nu_x = BH(absBx) / absBx
                    
                    absBy = np.abs(By)
                    if absBy == 0 : nu_y = BH(0.001) / 0.001
                ...
                if B2v != None:
                    ...
                    #dvdB2x = df1(Bx**2)
                    #dvdB2y = df1(By**2)
                    if absBx > 0: dvdB2x = self.fp(B2v, Bx**2)
                    if absBy > 0: dvdB2y = self.fp(B2v, By**2)
                    
                    if dvdB2x<0 or dvdB2y<0 :
                        print('dvdB2 is negative!!, B=[', Bx, By,']')
                
                # 이하 for문을 사용하지 않고 행렬 연산으로 한번에 계산할 수 있는지 고민해야 함!
                # python에서는 이중 for 루프는 지양해야 함!
                for n in range(dim):
                    #source term
                    #Q[n] = Q[n] + N[n]*J*detJ*w
                    ...
                    for m in range(dim):
                        ...
                        SS2[n][m] = SS2[n][m] + ((2*dvdB2x*Exn*Eym) + \
                                                 (2*dvdB2y*Eyn*Exm)) * detJ*w
                                               
                #SS1 = SS1 + np.matmul(gradNt, np.matmul(inv_mu, gradN)) * detJ*w
                                               
                # source term (Current Density and Magnetization)
                Nt = np.transpose(N)
                gradN = np.array([dNdx, dNdy])
                gradNt = np.transpose(gradN)
                M = np.array([nu_y*My, -nu_x*Mx])       # 자화벡터 ; 좌측 y, x 순서 및 부호 주의!
                
                Q = Q + (Nt*J - np.dot(gradNt, M)) * detJ*w
                
            # Q에 SS1*A 빼기
            Q = Q - np.matmul(e['SS'], A_prev)
            ...
            if B2v != None:
                ...
                #dvdB2x = df1(Bx**2)
                #dvdB2y = df1(By**2)
                dvdB2x = self.fp(B2v, Bx**2)
                dvdB2y = self.fp(B2v, By**2)
    ...
    def solve(self):
        ...
        while True:
            ...
            err = np.linalg.norm(dA)
            print('Iiteration: ', iter + 1, ', Error:',err)
            self.A_ = self.A_ + dA
            #print(dA)
            #print(self.A_)
            if (iter>0) and ((err<1e-5) or (iter>3)):
                break
