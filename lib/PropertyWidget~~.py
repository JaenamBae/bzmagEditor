...
class PropertyWidget(QDockWidget, Ui_PropertyWidget):
    ...
    def add_property(self, group, name, value, type, readonly):
        ...
        # type == vector2
            ...
            xy = value.split(',')
            pt = QPoint(float(xy[0]), float(xy[1]))
    ...
    # ------------------------------------------------------------------------        
    def build_cs(self, parent, idx = 0):
        if idx == 0:
            self.CSIDtoEnum_[-1] = idx
            self.EnumToCSID_[idx] = -1
            self.CSNames_.append('Global')
            idx = idx + 1
            
        for cs in parent.getChildren():
            self.CSIDtoEnum_[cs.getID()] = idx
            self.EnumToCSID_[idx] = cs.getID()
            self.CSNames_.append(cs.getAbsolutePath())
            idx = idx + 1
            
            self.build_cs(cs, idx)
    ...
    def build_ObjectItem(self, parent, IDtoEnum, EnumToID, Name, DefaultName, index = 0):
        ...
        for node in parent.getChildren():
            ...
            Name.append(node.getAbsolutePath())
            index = index + 1
